...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) { // if move(opcode) is not -1
				// TODO
				// after-state
				/*
				mv.set_value(move->reward() + estimate(move->afterstate()));
				if (move->value() > best->value()) best = mv;
				*/
				// after-state

				// before-state
				board after_state = move->after_state();
				float possible_val = 0;
				for(int i = 0; i < 16; i++){
					if(after_state.at(i) == 0){
						// 2 
						after_state.set(i, 1);
						possible_val += 0.9 * estimate(after_state);
						after_state = move->after_state(); // 
						// 4
						after_state.set(i, 2);
						possible_val += 0.1 * estimate(after_state);
						after_state = move->after_state(); // 
					}
				}
				move->set_value(move->reward() + possible_val);
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		for(path; path.size(); path.pop_back()){
			...
			/*
			move& mv = path.back();
			float error = target - estimate(st.afterstate());
			target = st.reward() + update(st.afterstate(), alpha * error);
			*/
			// after-state

			// before-state
			state& st = path.back();
			float error = st.reward() + target - estimate(st.before_state());
			target = update(st.before_state(), alpha * error);
			// before-state
		}
