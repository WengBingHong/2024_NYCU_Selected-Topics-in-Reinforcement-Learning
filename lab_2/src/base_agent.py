...
class DQNBaseAgent(ABC):
	...
	def train(self):
		...
		train_rewards_file = 'csv/train_rewards.csv'
		validation_rewards_file = 'csv/validation_rewards.csv'

		# Create or clear the files at the beginning of training
		if os.path.exists(train_rewards_file):
			os.remove(train_rewards_file)
		if os.path.exists(validation_rewards_file):
			os.remove(validation_rewards_file)

		with open(train_rewards_file, 'w', newline='') as train_file, open(validation_rewards_file, 'w', newline='') as val_file:
			train_writer = csv.writer(train_file)
			val_writer = csv.writer(val_file)

			# Write headers for the CSV files
			train_writer.writerow(['total_time_step', 'episode_reward'])
			val_writer.writerow(['total_time_step', 'avg_validation_reward'])
			...
			while self.total_time_step <= self.training_steps:
				...
				while True:
					...
					# Log the training reward after each episode by appending to CSV
					train_writer.writerow([self.total_time_step, episode_reward])
					train_file.flush()  # Ensure data is written to disk
				...
				if episode_idx % self.eval_interval == 0:
					...
					val_writer.writerow([self.total_time_step, avg_score])
					val_file.flush()  # Ensure data is written to disk
